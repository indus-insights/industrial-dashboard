...
# Charger et afficher le logo Nexans
import base64
...
<div class='main-header'>
    <div class='dashboard-title'>Daily Production Dashboard</div>
    <div class='dashboard-date'>{current_date.strftime('%d.%m.%Y')}</div>
</div>
""", unsafe_allow_html=True)

st.markdown("<div class='section-header'>🎯 QUALITY & COMPLAINTS</div>", unsafe_allow_html=True)
...
with col1:
    ...
    <div class="metric-card">
        <div class="metric-header">Customer Complaints</div>
        <div class="metric-value {color}">{kunde_mist:.0f}</div>
        <div class="metric-label">of {kunde_m_soll:.0f} monthly max</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
                <div class="metric-sub-value">{last_row['Kunde_TIST']:.0f}</div>
                <div class="metric-sub-label">Today</div>
            </div>
            <div class="metric-sub-item">
                <div class="metric-sub-value">{kunde_m_gap:+.0f}</div>
                <div class="metric-sub-label">Gap</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-header">Supplier Complaints</div>
        <div class="metric-value {color}">{liefer_mist:.0f}</div>
        <div class="metric-label">of {liefer_m_soll:.0f} monthly max</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
...
with col3:
    ...
    <div class="metric-card">
        ...
        <div class="metric-label">of {frtx_m_soll:.0f} monthly max</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
...
st.markdown("<div class='section-header'>🛡️ SAFETY, LOGISTICS & PERFORMANCE</div>", unsafe_allow_html=True)
...
with col4:
    ...
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-header">Risk Hunting</div>
        <div class="metric-value {color}">{last_row['RiskHunting_TIST']:.0f}</div>
        <div class="metric-label">risks identified (target: {risk_t_soll:.0f}/day)</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
            ...
            <div class="metric-sub-item">
                ...
                <div class="metric-sub-label">vs Target</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col5:
    ...
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-header">Inventory (Cycle Counting)</div>
        <div class="metric-value {color}">{cycle_tist:+.0f}€</div>
        <div class="metric-label">variance (limit: ±{cycle_grenze:.0f}€)</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
                <div class="metric-sub-value">{cycle_t_gap:+.0f}€</div>
                <div class="metric-sub-label">Excess</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col6:
    ...
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-header">Shipments (Week {last_week['Week']:.0f})</div>
        <div class="metric-value {color}">{last_week['WCompleted']:.0f}/{last_week['WTotal']:.0f}</div>
        <div class="metric-label">deliveries completed</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
...
with col7:
    ...
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-header">SAA (Standard Work)</div>
        <div class="metric-value {color}">{last_row['SAA_MIST']:.0f}%</div>
        <div class="metric-label">compliance (target: {saa_percent_soll:.0f}%)</div>
        <div class="metric-sub">
            <div class="metric-sub-item">
...
with col8:
    st.markdown("<div class='section-header'>📋 OPEN ACTIONS</div>", unsafe_allow_html=True)
...
with col9:
    st.markdown("<div class='section-header'>👥 ABSENCES</div>", unsafe_allow_html=True)
    ...
    <div class="metric-card" style="margin-top: 8px;">
        ...
        <div class="metric-label" style="font-size: 14px;">absences today</div>
        <div class="metric-sub" style="margin-top: 16px;">
            <div class="metric-sub-item">
                <div class="metric-sub-value">{abw_t_gap:+.0f}</div>
                <div class="metric-sub-label">vs Target ({abw_t_soll:.0f})</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)